	...
	if (tupsize < dataoff + sizeof(uint32))
		ereport(ERROR,
				(errcode(ERRCODE_INDEX_CORRUPTED),
				 errmsg("invalid hash index tuple size in index \"%s\"",
						RelationGetRelationName(state->rel)),
				 errdetail_internal("Index tid=(%u,%u) tuple size=%zu data offset=%zu.",
								 blkno, offnum, tupsize, dataoff)));
...
hash_check_split_tuples(HashCheckState *state)
{
	...
	while ((entry = hash_seq_search(&status)) != NULL)
	{
		if (!entry->found)
			ereport(ERROR,
					(errcode(ERRCODE_INDEX_CORRUPTED),
					 errmsg("hash index \"%s\" is missing a tuple copied by a bucket split",
							RelationGetRelationName(state->rel)),
					 errdetail_internal("Heap tid=(%u,%u), hash key=%u.",
